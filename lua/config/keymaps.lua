...
if user == "ianus" then
  function _G.ToggleGeminiTerminal()
    ...
    -- Find an existing terminal buffer and window marked as our gemini terminal
    for _, buf in ipairs(vim.api.nvim_list_bufs()) do
      if vim.api.nvim_buf_is_valid(buf) and vim.bo[buf].buftype == "terminal" and vim.b[buf].is_gemini_terminal then
        if vim.fn.jobwait({ vim.b[buf].terminal_job_id }, 0)[1] == -1 then
          gemini_buf_nr = buf
          gemini_win_id = vim.fn.bufwinid(buf)
          break
        end
      end
    end

    -- If the terminal is the current window, hide it
    if gemini_win_id > 0 and gemini_win_id == vim.api.nvim_get_current_win() then
      vim.cmd("hide")
      return
    end

    -- If the terminal window exists, focus it
    if gemini_win_id > 0 then
      vim.api.nvim_set_current_win(gemini_win_id)
    else
      -- Find project root
      local root = vim.fn.fnamemodify(vim.fn.finddir(".git", ".;", 1), ":h")
      if root == "" or root == "." then
        root = vim.fn.getcwd()
      end

      -- If the buffer exists but has no window, or doesn't exist at all
      vim.cmd("vsplit")
      vim.cmd("tcd " .. root)
